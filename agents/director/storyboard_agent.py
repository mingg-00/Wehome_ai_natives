...
from config.settings import ensure_directories, settings  # 설정에서 provider, 요청 타임아웃, 캐시 경로 등을 읽어옴
...
class StoryboardAgent:
    ...
    def load_cached_storyboard(self, input_data: dict[str, Any] | None = None) -> dict[str, Any] | None:
        ...
        print(f"[StoryboardAgent] Reused storyboard cache: {cache_path}", flush=True)
        return cached_storyboard
    ...
    def save_storyboard_cache(self, storyboard: dict[str, Any], input_data: dict[str, Any] | None = None) -> None:
        ...
        ensure_directories(os.path.dirname(cache_path))
        with open(cache_path, "w", encoding="utf-8") as file_handle:
            json.dump(storyboard, file_handle, ensure_ascii=False, indent=2)
        print(f"[StoryboardAgent] Saved storyboard cache: {cache_path}", flush=True)
    ...
    def _request_storyboard(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        for attempt in range(1, self.retry_attempts + 1):
            try:
                raw_text = self.provider.request_storyboard_text(prompt, input_data)
                break
                ...
                if status_code in retryable_statuses and attempt < self.retry_attempts:
                    ...
                    print(
                        f"[StoryboardAgent] {self.provider_name} request retry {attempt}/{self.retry_attempts} after HTTP {status_code}; waiting {wait_seconds:.1f}s",
                        flush=True,
                    )
                ...
                if response is not None:
                    raise StoryboardAPIError(
                        f"{self.provider_name} API request failed: {response.status_code} {response.reason}"
                    ) from exc
                raise StoryboardAPIError(f"{self.provider_name} API request failed: HTTP error") from exc
            except requests.RequestException as exc:
                if attempt < self.retry_attempts:
                    wait_seconds = self.retry_backoff_seconds * attempt
                    print(
                        f"[StoryboardAgent] {self.provider_name} request retry {attempt}/{self.retry_attempts} after {exc.__class__.__name__}; waiting {wait_seconds:.1f}s",
                        flush=True,
                    )
                    time.sleep(wait_seconds)
                    last_error = exc
                    continue
                raise StoryboardAPIError(f"{self.provider_name} API request failed: {exc.__class__.__name__}") from exc
            except RuntimeError as exc:
                raise StoryboardAPIError(str(exc)) from exc

        else:
            if last_error is not None:
                raise StoryboardAPIError(f"{self.provider_name} API request failed after {self.retry_attempts} attempts: {last_error}") from last_error
            raise StoryboardAPIError(f"{self.provider_name} API request failed after {self.retry_attempts} attempts.")
    ...
    def parse_storyboard_text(self, raw_text: str, allowed_assets: list[str] | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            raise StoryboardSchemaError(f"Storyboard response is not valid JSON: {exc}") from exc
    ...
    def validate_storyboard_schema(
        ...
        if not isinstance(storyboard, dict):
            raise StoryboardSchemaError("Storyboard must be a JSON object.")

        required_top_level_keys = {"video_metadata", "scenes", "recommended_hashtags"}
        missing_keys = required_top_level_keys - set(storyboard)
        if missing_keys:
            raise StoryboardSchemaError(f"Storyboard is missing keys: {sorted(missing_keys)}")
        ...
        if not isinstance(video_metadata, dict):
            raise StoryboardSchemaError("video_metadata must be an object.")
        if not isinstance(scenes, list) or not scenes:
            raise StoryboardSchemaError("scenes must be a non-empty list.")
        if not isinstance(hashtags, list):
            raise StoryboardSchemaError("recommended_hashtags must be a list.")
        ...
        for index, scene in enumerate(scenes):
            ...
            if not isinstance(scene, dict):
                raise StoryboardSchemaError(f"Scene {index + 1} must be an object.")
            missing_scene_keys = required_scene_keys - set(scene)
            if missing_scene_keys:
                raise StoryboardSchemaError(
                    f"Scene {index + 1} is missing keys: {sorted(missing_scene_keys)}"
                )
        ...
        for scene in scenes:
            ...
            if not isinstance(scene.get("scene_number"), int):
                raise StoryboardSchemaError("scene_number must be an integer.")
    ...
    def _ensure_string_fields(self, data: dict[str, Any], field_names: list[str]) -> None:
        for field_name in field_names:
            if not isinstance(data.get(field_name), str):
                raise StoryboardSchemaError(f"{field_name} must be a string.")
